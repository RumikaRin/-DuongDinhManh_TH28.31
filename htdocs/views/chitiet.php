...
$sql = "SELECT id_sp, anh_sp, ten_sp, gia_sp, chitiet_sp, tacgia_sp FROM sanpham WHERE id_sp = ?";
$stmt = $ketnoi->prepare($sql);

if ($stmt === false) {
    error_log("Lỗi chuẩn bị truy vấn trong chitiet.php: " . $ketnoi->error);
...
if ($result->num_rows > 0) {
    ...
    $imgPath = htmlspecialchars($row["anh_sp"]);
...
<div class="flof-container fade-in-up">
    <div style="margin-bottom: 24px;">
        <a href="index.php?go=sanpham" class="flof-btn" style="background: white; border: 1px solid var(--warm-200); padding: 8px 16px; text-decoration: none; color: var(--warm-700);">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right: 8px;"><line x1="19" y1="12" x2="5" y2="12"></line><polyline points="12 19 5 12 12 5"></polyline></svg> Quay lại danh sách
        </a>
    </div>
    ...
    <div class="flof-grid-12" style="margin-bottom: 40px;">
        ...
        <div class="flof-col-5">
            <div class="flof-card" style="padding: 0; overflow: hidden; position: sticky; top: 100px; background: white; text-align: center; border-radius: var(--radius-xl);">
                <img src="<?php echo $imgPath; ?>" alt="<?php echo htmlspecialchars($row["ten_sp"]); ?>" style="width: 100%; max-height: 600px; object-fit: contain; display: block; margin: 0 auto;">
            </div>
        </div>
            ...
            <div class="flof-card">
                ...
                <div style="display: flex; flex-wrap: wrap; gap: 16px; margin-bottom: 24px; color: var(--warm-600); font-size: 0.95rem;">
                    <div style="display: flex; align-items: center; gap: 6px;">
                        ...
                        <strong>Tác giả:</strong> <?php echo htmlspecialchars($row["tacgia_sp"]); ?>
                    </div>
                    <?php
                    $cat_query = "SELECT d.ten_dm FROM danhmuc d 
                                   INNER JOIN sanpham_danhmuc sd ON d.id_dm = sd.id_dm 
                                   WHERE sd.id_sp = " . intval($row['id_sp']);
                    ...
                    // Rating
                    $avg = 0; $cnt = 0;
                    $rs = $ketnoi->query("SELECT AVG(rating) AS avg_rating, COUNT(*) AS cnt FROM comments WHERE product_type='sanpham' AND product_id=".(int)$row['id_sp']." AND status='approved'");
                    if ($rs && $rs->num_rows) { 
                        $d = $rs->fetch_assoc(); 
                ...
                <div style="font-size: 2.2rem; font-weight: 800; color: #d32f2f; margin-bottom: 32px; padding: 16px; background: var(--warm-50); border-radius: var(--radius-xl); border-left: 4px solid #d32f2f;">
                ...
                <div style="margin-top: 40px;">
                    ...
                    <div style="color: var(--warm-700); line-height: 1.8; font-size: 1rem;">
                        <?php echo nl2br(htmlspecialchars($row["chitiet_sp"])); ?>
                    </div>
                </div>
            </div>
        </div>
    ...
    $qc = $ketnoi->query("SELECT rating, COUNT(*) AS c FROM comments WHERE product_type='sanpham' AND product_id=".(int)$row['id_sp']." AND status='approved' GROUP BY rating");
    if ($qc) { 
    ...
    // List comments
    $cmt = $ketnoi->query("SELECT username, rating, content, created_at, reply FROM comments WHERE product_type='sanpham' AND product_id=".(int)$row['id_sp']." AND status='approved' ORDER BY id DESC LIMIT 50");
    ...
    if (isset($_SESSION['ten_tv'])) {
        ...
        echo app_csrf_field();
        echo "<input type='hidden' name='product_type' value='sanpham'>";
        echo "<input type='hidden' name='product_id' value='".(int)$row['id_sp']."'>";
    ...
    // Lấy sản phẩm thường (không trùng với sản phẩm hiện tại)
    $rel1 = $ketnoi->query("SELECT id_sp, ten_sp, anh_sp, gia_sp, 'normal' as type FROM sanpham WHERE id_sp<>".(int)$row['id_sp']." ORDER BY RAND($randomSeed) LIMIT 6");
    if ($rel1) {
    ...
    // Lấy sản phẩm sale
    $rel2 = $ketnoi->query("SELECT id_tt as id_sp, ten_tt as ten_sp, anh_tt as anh_sp, saugiamgia_tt as gia_sp, giasp_tt, giamgia_tt, 'sale' as type FROM sales ORDER BY RAND($randomSeed) LIMIT 6");
    if ($rel2) {
